...
RUN apt update && apt install wget -y && mkdir -p /opt/docker-misp && cd /opt/ && wget -cO ${MISP_TAG}.tar.gz https://github.com/mkilijanek/misp-server/archive/refs/tags/${MISP_TAG}.tar.gz && tar xvf misp-server-${MISP_TAG}.tar.gz -C /opt && cp -r /opt/misp-server-${MISP_TAG}/* /opt/docker-misp/ 
